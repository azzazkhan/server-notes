...
# Laravel Forge
ssh-rsa  worker@forge.laravel.com
...
response=`curl -s -w "%{http_code}" POST --url https://forge.laravel.com/servers/830101/vps/connection --data-urlencode "forge_token="`
...
echo "colorful-surf" > /etc/hostname
sed -i 's/127\.0\.0\.1.*localhost/127.0.0.1	colorful-surf.localdomain colorful-surf localhost/' /etc/hosts
hostname colorful-surf
...
PASSWORD=$(mkpasswd -m sha-512 DR8tfBF4KEP7XNd1mEJs)
...
debconf-set-selections <<< "mysql-community-server mysql-community-server/root-pass password D84C8fx8WQRZCw6XcqhV"
debconf-set-selections <<< "mysql-community-server mysql-community-server/re-root-pass password D84C8fx8WQRZCw6XcqhV"
...
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "CREATE USER 'root'@'1.1.1.1' IDENTIFIED BY 'D84C8fx8WQRZCw6XcqhV';"
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "CREATE USER 'root'@'%' IDENTIFIED BY 'D84C8fx8WQRZCw6XcqhV';"
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "GRANT ALL PRIVILEGES ON *.* TO root@'1.1.1.1' WITH GRANT OPTION;"
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "GRANT ALL PRIVILEGES ON *.* TO root@'%' WITH GRANT OPTION;"
service mysql restart

mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "CREATE USER 'forge'@'1.1.1.1' IDENTIFIED BY 'D84C8fx8WQRZCw6XcqhV';"
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "CREATE USER 'forge'@'%' IDENTIFIED BY 'D84C8fx8WQRZCw6XcqhV';"
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "GRANT ALL PRIVILEGES ON *.* TO 'forge'@'1.1.1.1' WITH GRANT OPTION;"
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "GRANT ALL PRIVILEGES ON *.* TO 'forge'@'%' WITH GRANT OPTION;"
mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "FLUSH PRIVILEGES;"

# Create The Initial Database If Specified

mysql --user="root" --password="D84C8fx8WQRZCw6XcqhV" -e "CREATE DATABASE forge CHARACTER SET utf8 COLLATE utf8_unicode_ci;"
...
curl --insecure --data "event_id=61674432&server_id=830101&recipe_id=" https://forge.laravel.com/provisioning/callback/app
